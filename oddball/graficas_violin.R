...
# 4. c) Regresiones SSIS ####

library(tidymodels)

ssis_reg <- pelis_long %>% 
  filter(Tipo == "SSIS") %>% 
  pivot_wider(names_from = pre.post, values_from = valor)
...
lm_fit <- lm_model %>%
  fit(Post ~ Pre, data = ssis_reg)

# lm_fit <- lm_model %>%
#   fit(Post ~ Pre, data = ssis_reg %>% filter(Grupo == "Identidad"))

# lm_fit <- lm_model %>% 
#   fit(valor ~ pre.post, data = ssis_reg %>% filter(Grupo == "Identidad"))

summary(lm_fit$fit)

lm_fit_subescala <- lm_model %>%
  fit(value ~ pre.post + Subescala + Grupo,
      data = SSIS_long %>% 
        filter(escala == "Habilidades sociales"))

summary(lm_fit_subescala$fit)

ssis_subescala_reg <- SSIS_long %>% 
  pivot_wider(names_from = pre.post, values_from = value)

lm_fit_subescala <- lm_model %>%
  fit(Post ~ Pre + Subescala + Grupo,
      data = ssis_subescala_reg %>% 
        filter(escala == "Habilidades sociales"))
...
lm_fit_subescala <- lm_model %>%
  fit(Post ~ Pre + Subescala + Grupo,
      data = ssis_subescala_reg %>% 
        filter(escala == "Problemas de conducta"))

summary(lm_fit_subescala$fit)

lm_fit_subescala <- lm_model %>%
  fit(Post ~ Pre + Subescala,
      data = ssis_subescala_reg)
